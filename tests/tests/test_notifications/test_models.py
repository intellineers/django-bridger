...
class TestNotification:
    def setup_method(self):
        self.user = get_user_model().objects.create(username="test_user", password="ABC", is_active=True, is_superuser=True)

    def test_str(self, mocker):
        mocker.patch("celery.execute.send_task")

        notification = Notification.objects.create(
            recipient=self.user, title="New Notification", message="You have a new notification",
        )
